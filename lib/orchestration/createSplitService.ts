 ...
 * @returns splitId
 */
export async function updateSplitFlow(id: string, input: NewSplitInput) {
  validateSplit(input);
  // getSplitInformation enriches with exerciseData, etc.
  const enriched = await getSplitInformation({ ...input, id } as unknown as Split);
  await updateSplitById(id, enriched);
  return id;
}

/**
 * Loads a split for editing
 * @param id Split ID
 * @returns Split data
 */
export async function loadSplitForEdit(id: string): Promise<Split | null> {
  const split = await getSplitBySplitId(id);
  if (!split) return null;
  // Enrich with exercise data
  return await getSplitInformation(split);
}
